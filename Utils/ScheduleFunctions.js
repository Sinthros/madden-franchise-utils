...
// Required lookups
const teamLookupPath = path.join(__dirname, '../retroSchedules/teamLookup.json');
...
async function convertScheduleToJson(franchise) {
	...
	{ 
		...
		for (let j = 0; j < numRowsSeasonGame; j++)
		{
			...
			// If we have 16 games, the week must be complete, so no need to continue iterating, move on to the next week
