...
const allCoachVisuals = JSON.parse(fs.readFileSync(path.join(__dirname, './25/coachVisualsLookup.json'), 'utf8'));
const baseCoachVisualJson = JSON.parse(fs.readFileSync(path.join(__dirname, './25/baseCoachVisualLookup.json'), 'utf8'));
...
async function getCoachValues(coachTable, i) {
  ...
  if (skinToneValue !== null) {
    ...
    skinTone = !isNaN(lastDigit) ? lastDigit : 1; //If it's a digit, keep the value, else default to skin tone 1
    genericHeadName = genericHead;
  }
  else {
    genericHeadName = coachTable.records[i].GenericHeadAssetName;
    const firstChar = genericHeadName.charAt(0); // Get the character at index 0
    const tenthChar = genericHeadName.charAt(10); // Get the character at index 10

    skinTone = !isNaN(firstChar) ? parseInt(firstChar) 
                    : !isNaN(tenthChar) ? parseInt(tenthChar) 
                    : 1;
  }
...
async function updateCoachVisuals(coachValues, jsonToUpdate,visualMorphKeys, size = "N/A", coachLookup = null) {
  ...
  const coachOnFieldLoadout = jsonToUpdate.loadouts.find(
    loadout => loadout.loadoutType === "CoachOnField" || loadout.loadoutType === "Misc"
  );
