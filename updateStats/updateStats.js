...
const DEFAULT_COLUMNS = [
  ...
  "DEFOUTSIDERUN_YARDS",
  "DEFSHORTPASS_YARDS",
  "DLINEFUMBLERECOVERYYARDS",
  "DSECINTLONGESTRETURN",
  "DSECINTRETURNYARDS",
  "INSIDERUN_YARDS",
  "INSIDEPOCKETTHROW_YARDS",
  "KRETLONGEST",
  "KRETYARDS",
  "MEDIUMPASS_YARDS",
  "OUTSIDEPOCKETTHROW_YARDS",
  "OUTSIDERUN_YARDS",
  "PASSLONGEST",
  "PASSYARDS",
  "PRETLONGEST",
...
function getTeamShortName(teamTable, teamIndex) {
  ...
  const record = teamTable.records.find(
    (record) =>
      !record.isEmpty && !FranchiseUtils.NFL_CONFERENCES.includes(record.DisplayName) && record.TeamIndex === teamIndex,
  );
...
async function processPlayersInBatches(players, isCareer, batchSize = 5) {
  ...
  const playersToProcess = players.filter(
    (player) => !(player.PLAYERLINK in LOOKUP_FILE) && parseInt(player.MAXYEAR, 10) >= 2000,
  );
...
async function updateStats(franchise, isCareer) {
  ...
  for (const player of playersInLookupFile) {
    ...
    if (
      !playerRecord ||
      FranchiseUtils.OLINE_POSITIONS.includes(playerRecord.Position) ||
      playerRecord.Position === "LS"
    )
      continue;
...
function getPlayerRecord(playerTable, assetName) {
  return playerTable.records.find(
...
async function checkValidPlayers(matches, skippedPlayers, teamIndices, pfrObject, isCareer, colleges) {
  ...
  for (const player of matches) {
    ...
    const ageMatch = !isCareer
      ? pfrObject.finalPfrAge === record.Age
      : Math.abs(pfrObject.finalPfrAge - record.Age) <= 1;
    ...
    if (isMatch || otherMatch) {
      ...
      console.log(
        `PFR: ${pfrObject.finalPfrName}, ${pfrObject.finalPfrAge}, ${pfrObject.pfrPlayer.TEAM}. Last played in ${pfrObject.finalSeason}. ${pfrObject.pfrPlayer.PLAYERLINK}.\nMadden: ${finalMaddenName}, ${record.Age}, ${record.Position}, for the ${teamName}. ${playerCollege}. ${record.YearsPro} years of experience.`,
      );
...
function isNumericField(record, column) {
  const field = record._fields[column]?.offset;
  if (!field) return false;

  const { type } = field;

  return type === "int" || type === "s_int" || type === "uint" || type === "float";
}

async function clearCurrentSeasonStats(franchise) {
  const playerTable = franchise.getTableByUniqueId(tables.playerTable);
  const seasonInfoTable = franchise.getTableByUniqueId(tables.seasonInfoTable);
  ...
  for (const playerRecord of playerTable.records) {
    if (!FranchiseUtils.isValidPlayer(playerRecord)) continue;

    const statsRef = playerRecord.SeasonStats;
    if (statsRef === FranchiseUtils.ZERO_REF) continue;

    const { row, tableId } = FranchiseUtils.getRowAndTableIdFromRef(statsRef);
    const seasonStatsArray = franchise.getTableById(tableId);
    await seasonStatsArray.readRecords();

    const statsRecord = seasonStatsArray.records[row];
    const columns = FranchiseUtils.getColumnNames(statsRecord);

    for (const column of columns) {
      if (FranchiseUtils.isFtcReference(statsRecord, column) || !FranchiseUtils.isReferenceColumn(statsRecord, column))
        continue;

      const ref = statsRecord[column];
      if (ref === FranchiseUtils.ZERO_REF) continue;

      const { row: statRow, tableId: statTableId } = FranchiseUtils.getRowAndTableIdFromRef(ref);

      const statTable = franchise.getTableById(statTableId);
      await statTable.readRecords();

      const statRecord = statTable.records[statRow];
      if (!statRecord || statRecord.SEAS_YEAR !== currentYear) continue;

      const statColumns = FranchiseUtils.getColumnNames(statRecord);

      for (const statColumn of statColumns) {
        if (["SEAS_YEAR", "TeamPrefixName", "STAT_KEEP", "YEARBYYEARTEAMINDEX"].includes(statColumn)) continue;

        if (!isNumericField(statRecord, statColumn)) continue;

        statRecord[statColumn] = 0;
      }
    }
  }
}
...
franchise.on("ready", async function () {
  ...
  await clearCurrentSeasonStats(franchise);
