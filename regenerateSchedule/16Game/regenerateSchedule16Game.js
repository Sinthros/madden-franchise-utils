...
console.log(`This program will allow you to regenerate the 16 game schedule in your franchise file to be closer to a real NFL schedule. This tool must be run during the preseason. Madden ${FranchiseUtils.formatListString(validGameYears)} are supported.\n`)
...
franchise.on('ready', async function () {
	...
	fs.copyFileSync(path.join(__dirname, 'HW12-16Game.exe'), './HW12-16Game.exe');
	...
	if(fs.existsSync('solution.json'))
	{
		...
		if(!useOld)
		{
			...
			runProgram('HW12-16Game.exe > GeneratorLog.txt');
