...
console.log("In this program, you can convert your Madden 25 Franchise to Madden 26 (Or Madden 26 to another Madden 26 Franchise File)");
...
const sourceFranchise = FranchiseUtils.init(SOURCE_VALID_YEARS, {customYearMessage: "Select the Madden version of your SOURCE Franchise file. Valid inputs are 25 and 26.", promptForBackup: false, isAutoUnemptyEnabled: false});
...
function isDecrementYear() {
  return is25To26 && transferSeasonYear && !useBaseCalendar;
}
...
function handlePlayerTable(playerTable) {
  for (const record of playerTable.records) {
    ...
    if (isDecrementYear()) {
      record.YearDrafted--; // If transferring from 25 to 26, we need to account for the year difference
    ...
    /*const {newOverall, newArchetype} = FranchiseUtils.calculateBestOverall(record, 26);
...
async function handleCharacterVisuals(sourceRecord, targetRecord, currentTableName) {
  ...
  if (characterVisuals !== FranchiseUtils.ZERO_REF) {
    ...
    if (is25To26) {
      ...
      try {
        jsonData = JSON.parse(visualsTable.records[visualsMetadata.row].RawData);
      } catch (error) {
        console.error(error);
        return;
      }
...
async function recalculatePlayerOverall() {
  const playerTable = targetFranchise.getTableByUniqueId(TARGET_TABLES.playerTable);
  await playerTable.readRecords();

  for (const record of playerTable.records) {
    if (FranchiseUtils.isValidPlayer(record, { includeDraftPlayers: true}))
    {
      const { newOverall, newArchetype } = FranchiseUtils.calculateBestOverall(record, 26);
      record.OverallRating = newOverall;
      record.PlayerType = newArchetype;
